...
contract LandCrowdSale {
    ...
    function onLonNFT(uint256 _nftID, uint256 _blockId) public {
        NFTSharesStruct storage _nftSharesDetails =
            sharesdetails[_nftID][_blockId - 1];
        _nftSharesDetails.onLoan = true;
        _nftSharesDetails.loanDeadLine = block.timestamp + 30 days;

        NFTSharesStruct memory _nftSharesDetails2 =
            usersSharesdetails[_nftID][_nftSharesDetails.owner][_blockId - 1];
        usersSharesdetails[_nftID][_nftSharesDetails.owner][
            _blockId - 1
        ] = _nftSharesDetails2;
    }
}
